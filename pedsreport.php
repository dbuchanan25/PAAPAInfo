...
{
     echo'
           <form method="post" action="pedsreport.php">
           <table align="center" border="0" width="900" bordercolor="#000000">
              <tr>
		         <td width="300">
				 </td>
		         <td width="300" align="center"><h2>Create Peds Extra Hours Report For Month:</h2>
				 </td>
		         <td width="300">
				 </td>
		      </tr>
...
{   
   ...
   */
   ...
   //mysql_select_db("anesthesia_pay", $con);
   
   
   $s = "SELECT * from monthassignment where monthnumber={$_REQUEST['mnt']} AND yearnumber={$_REQUEST['yar']} AND (assignment='Peds Call' OR assignment='Peds Call Short')";
   $sq = mysql_query($s);
   
   echo '<table align="left" border="1">
   
       <h1 align="center">
	   Peds Hours Report For: '.$_REQUEST["mnt"].'/'.$_REQUEST["yar"].'
	   </h1>';
   ...
   $hs = "SELECT * FROM monthassignment WHERE mdnumber={$sr[0]} AND monthnumber={$sr[1]} AND daynumber={$sr[2]} AND yearnumber={$_REQUEST['yar']} AND assigntype=4";
   $hsq = mysql_query($hs);
   
   while ($hsqr = mysql_fetch_row($hsq))
   {
      echo '<td>'.$hsqr[4].'</td><td>'.$hsqr[7].'</td><td>'.$hsqr[9].'</td>';
   }
