...
for ($x=$_SESSION['bdoy']; $x<=$_SESSION['edoy']; $x++)
{
	...
	$vacinfoq = "SELECT * FROM vacation WHERE 
										anesthesiologistinitials='{$_SESSION['draftinitials']}' 
	...
	$vacinfopriority = $vacinforesult[6];
	$vacinfocredited = $vacinforesult[8];
	
	if ($vacinforesult[8] == -1)
	{
		$calreplaceq = "UPDATE calendar2011 SET waitlist = $dayresult[7]-1 
		                                    WHERE dayofyear = $x";
		$calreplaceresult2 = mysql_query($calreplaceq);
	}
	else if ($vacinforesult[8] == 0)
	{
		$calreplaceq1 = "UPDATE calendar2011 SET uncredited = $dayresult[5]+1
											 WHERE dayofyear = $x";
		$calreplaceresult1 = mysql_query($calreplaceq1);
		$calreplaceq2 = "UPDATE calendar2011 SET uncreditedtaken = $dayresult[6]-1
		                                     WHERE dayofyear = $x";
		$calreplaceresult2 = mysql_query($calreplaceq2);
	}
	else if ($vacinforesult[8] == 1)
	{
		$calreplaceq1 = "UPDATE calendar2011 SET credited = $dayresult[3]+1
		                                     WHERE dayofyear = $x";
		$calreplaceresult1 = mysql_query($calreplaceq1);
		$calreplaceq2 = "UPDATE calendar2011 SET creditedtaken = $dayresult[4]-1
		                                     WHERE dayofyear = $x";
		$calreplaceresult2 = mysql_query($calreplaceq2);
	}
	...
	$priorityresult = 1;
	$increasepriorityq = "SELECT * FROM vacation WHERE 
												 month=$dayresult[1] AND
							  			         day=$dayresult[2]";
	$increasepriorityquery = mysql_query($increasepriorityq);
	while ($increasepriorityresult=mysql_fetch_row($increasepriorityquery))
	{
		if ($increasepriorityresult[6]>$vacinforesult[6])
		{
			$ipq = "UPDATE vacation SET priority=$increasepriorityresult[6]-1
			        WHERE number=$increasepriorityresult[10]";
			$ipresult = mysql_query($ipq);
			if (!ipresult)
				$priorityresult=0;
		}
	}
    ...
    //"vacation" table.                                         //
    //////////////////////////////////////////////////////////////
	$okdeleted = mysql_query ("DELETE FROM vacation WHERE
							  anesthesiologistinitials='{$_SESSION['draftinitials']}'
							  AND month=$dayresult[1] 
							  AND day=$dayresult[2]"
							 );
...
<form method=post action='quickentry.php' name=f1> 
<table align="center">
   <tr>
      <td align="center">
         <font size=6>
...
else
{
	...
	<table align=center>
	   ...
	   <tr>
	      <td align=center>
	<a href="quickentry.php">Try Again</a>
	      </td>
	   </tr>
	</table>';
	exit;	
...
//obtained.                                                                 //
//////////////////////////////////////////////////////////////////////////////	
	$begindayofyearquery = "SELECT dayofyear FROM calendar2011 WHERE 
	                        month='$beginmonth' AND day='$beginday'";
	$enddayofyearquery = "SELECT dayofyear FROM calendar2011 WHERE 
	                      month='$endmonth' AND day='$endday'";
...
///////////////////////////////////////////////////////////////////////////////		
		 ...
		 || (($datetime->format('m') >= $monthbegin) && ($datetime->format('Y') == 2011)))
		{
			...
			$_SESSION['firstpagequickentry']=1;
			echo'
			<form method="post" action="quickentry.php" class="input">
			    <tr>
	      			<td align="center">
...
/////////////////////////////////////////////////////////////////////////////////////////////
			...
			if ($available==0 && $repeatvacday==0)
			{
				...
				<table align="center">
					<tr>
						<th>		
						At least one of the days you are requesting is not available for 
						vacation.
						</th>
					</tr>
					<tr>
						<td align="center">
						If you want to add your name to the wait list for this/these day(s), 
						click on "Wait List".
						</td>
					</tr>
					<tr>
			...
			$_SESSION['firstpagequickentry']=1;
		        ...
		        <form method="post" action="vac_add_waitlist_draft.php" class="input">
				    <tr>
		      			<td align="center">
		      				<input type="submit" name="wait" value="Wait List" class="btn">
		          		</td>
		          	</tr>
		        </form>
				</table>';
...
*/
  ...
  include ('vacation_daydisplay_architecture.php');
  $datetime = new DateTime("now", new DateTimeZone('US/Eastern'));
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 ...
 echo'
    ...
    <tr>
     <td bgcolor="#ffffff" width="100%" height="50" align="center" style="font-size:large">'.$monthname.'</td>
     </tr>
    </table>
  ...
  $firstdayofweek = $datetime->format('N');
   switch ($firstdayofweek)
   {
     ...
     echo'
	 <tr>  
	 <td bgcolor="#ffffff" width="150" height="150" align="center">
        <table border="0" cellspacing="0" cellpadding="0">
		  <tr>
   ...
   vacation_daydisplay_architecture($numericaldayofmonth, $mno);
   $numericaldayofmonth++;
...
{
    echo'
    <td bgcolor="black" width="150" height="150" align="center">
        <table border="0" cellspacing="0" cellpadding="0">
		  <tr>
          ...
          <tr>
		   <td bgcolor="black" height="130" width="150">	   
		   </td>
          </tr>
		</table>
	 </td>
